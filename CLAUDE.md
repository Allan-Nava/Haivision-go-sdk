...
- **Aggiungere un metodo a `IHaivisionClient` è `major`, non `minor`.** In Go un'interfaccia esportata è un contratto a due vie: chi la implementa (i mock nei test dei consumer) non compila più. Vale anche per le feature "solo aggiuntive" — è la ragione per cui `route-update-delete` sta in `v2.0.0`.
